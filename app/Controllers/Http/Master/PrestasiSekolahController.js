...
const PrestasiSekolah = use('App/Models/Master/PrestasiSekolah')
const Database = use('Database') // (Adonis 4.x)
const Env = use('Env') // for Adonis 4.x

class PrestasiSekolahController {
 async index({ request, response }) {
    ...
    const dataQuery = PrestasiSekolah.query()
    ...
    result.data = result.data.map(data => {
      return {
        ...
        image: data.image
          ? `${baseUrl}/uploads/prestasisekolah/${data.image}`
          : null
      }
    })
  ...
  async store({ request, response }) {
    try {
      ...
      if (image) {
        ...
        await image.move('public/uploads/prestasisekolah', {
          name: fileName,
          overwrite: true
        })
        data.image = fileName
      ...
      data.is_active = 1
      const dataPrestasi = await PrestasiSekolah.create(data)
  ...
  async delete({ request, response, auth }) {
    try {
      ...
      const data = await PrestasiSekolah.findOrFail(id)
  ...
  async update({ request, response }) {
    try {
      ...
      const data = await PrestasiSekolah.find(id)
      if (!data) {
...
module.exports = PrestasiSekolahController
