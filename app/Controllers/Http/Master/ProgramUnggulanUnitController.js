...
class ProgramUnggulanUnitController {
 async index({ request, response }) {
    ...
    const dataQuery = ProgramUnggulanUnit.query().whereRaw('MD5(sekolah_id) = ?',sekolah_id)
    ...
    result.data = result.data.map(data => {
      return {
        ...
        image: data.image
          ? `${baseUrl}/uploads/programunit/${data.image}`
          : null
      }
    })
  ...
  async store({ request, response }) {
    try {
      ...
      const data = request.only(['name','deskripsi','sekolah_id'])
      ...
      if (image) {
        ...
        await image.move('public/uploads/programunit', {
          name: fileName,
          overwrite: true
        })
        data.image = fileName
      ...
      const dataPrestasi = await ProgramUnggulanUnit.create(data)
  ...
  async delete({ request, response, auth }) {
    try {
      ...
      const data = await ProgramUnggulanUnit.findOrFail(id)
  ...
  async update({ request, response }) {
    try {
      ...
      const data = await ProgramUnggulanUnit.find(id)
      if (!data) {
...
module.exports = ProgramUnggulanUnitController
