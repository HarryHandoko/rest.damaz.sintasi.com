...
class RegisterController {
  async create({ request, response, auth }) {
    ...
    try {
      ...
      const periodik = `${tahunAwal.toString().slice(2)}${tahunAkhir
        .toString()
        .slice(2)}`;
  ...
  async updateForm({ request, response, auth }) {
    ...
    try {
      ...
      if (request.input("step") == "1") {
        const SekolahData = await Sekolah.query()
        ...
        if (
          umur < SekolahData.batas_usia_min ||
          umur > SekolahData.batas_usia_max
        ) {
          return response.status(500).json({
            status: umur < SekolahData.batas_usia_min,
            message:
              "Usia Calon Peserta Didik tidak memenuhi syarat batas minimum dan maximum usia",
          });
        }
        ...
        const getSiswaPPDB = await SiswaPpdb.query()
          .where("id", updatePPDB.siswa_id)
          .first();
        updatePPDB.asal_sekolah =
          request.input("asal_sekolah") == "null"
            ? "-"
            : request.input("asal_sekolah");
        updatePPDB.jenjang_terakhir =
          request.input("jenjang_terakhir") == "null"
            ? "-"
            : request.input("jenjang_terakhir");
        getSiswaPPDB.nama_depan = request.input("nama_depan");
        getSiswaPPDB.nama_belakang = request.input("nama_belakang");
        ...
        getSiswaPPDB.hobby =
          request.input("hobby") == "null" ? "-" : request.input("hobby");
        ...
        getSiswaPPDB.riwayat_kesehatan =
          request.input("riwayat_kesehatan") == "null"
            ? "-"
            : request.input("riwayat_kesehatan");
        getSiswaPPDB.no_handphone =
          request.input("no_handphone") == "null"
            ? "-"
            : request.input("no_handphone");
        getSiswaPPDB.tgl_lahir = request.input("tgl_lahir");
    ...
    try {
      ...
      await Promise.all(
        dataRegis.map(async (item, index) => {
          ...
          siswa.usia = usia;
          siswa.tgl_lahir = formatDate(siswa.tgl_lahir+' 00:00:00');
          ...
          dataRegis[index].registrasi_ulang =
            dataRegistrasiUlang?.toJSON() || null;
  ...
  async delete({ request, response }) {
    try {
      const { code_pendaftaran, siswa_id } = request.only([
        "code_pendaftaran",
        "siswa_id",
      ]);
      ...
      const dataAward = await SiswaAward.query()
        .where("siswa_id", dataSiswa.id)
        .first();
      const dataAddress = await RegAddress.query()
        .where("register_id", data.id)
        .first();
      const dataParrent = await RegParent.query()
        .where("register_id", data.id)
        .first();
      const dataPayment = await Payment.query()
        .where("register_id", data.id)
    ...
    try {
      ...
      if (type === "Approve") {
        ...
        return response.status(400).json({
          success: false,
          message:
            'Tipe approval tidak valid. Gunakan "Approve" atau "Reject".',
        });
      }
  ...
  async daftarUlang({ request, response, auth }) {
    try {
      ...
      // Query pada transaction
      const lastRegister = await DaftarUlang.query()
  ...
  async payRegUlang({ request, response }) {
    try {
      const data = await DaftarUlang.query()
        .where("code_registrasi_ulang", request.input("code_registrasi_ulang"))
        .first();
      if (!data) {
        return response.status(500).json({
          success: false,
      ...
      if (fileKartuKeluarga) {
        const fileName = `${Date.now()}.${fileKartuKeluarga.extname}`;
        await fileKartuKeluarga.move(
          "public/uploads/ppdb/bukti_pembayaran_reg_ulang",
          {
            name: fileName,
            overwrite: true,
          }
  ...
  async statistikPendaftarUlang({ response }) {
    try {
      ...
      const totalDiterima = await DaftarUlang.query()
        .where("status_pembayaran", "01")
        .count();
      const totalDitolak = await DaftarUlang.query()
        .where("status_pembayaran", "02")
        .count();
      const totalDalamProses = await DaftarUlang.query()
        .where("status_pembayaran", "00")
        .count();
  ...
  async ApprovalRegisUlang({ response, request, auth }) {
    try {
      ...
      if (!data) {
        ...
        const dataRegist = await RegisterPPDB.query()
        ...
        const dataSekolah = await Sekolah.query()
          .where("id", dataRegist.sekolah_id)
          .firstOrFail();
        const dataSekolahGrade = await SekolahGrade.query()
          .where("id", dataRegist.grade_id)
          .firstOrFail();
        const dataSiswaAddress = await RegAddress.query()
          .select(
            "tbl_register_ppdb_addresses.*",
  ...
  async generateKTSPDF({ request, response }) {
    try {
      ...
      const formattedTanggalLahir = moment(siswa.tgl_lahir)
        .locale("id")
        .format("D MMMM YYYY");
  ...
  async updateTanggalTest({ request, response }) {
    try {
      const { code_ppdb, tgl_test } = request.only(["code_ppdb", "tgl_test"]);
      const register = await RegisterPPDB.query()
        .where("code_pendaftaran", code_ppdb)
        .first();
      if (!register) {
